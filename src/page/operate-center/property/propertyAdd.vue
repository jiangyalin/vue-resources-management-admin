...
<script type="text/ecmascript-6">
  ...
  const EditBedSetting = vue => {
    const bedSetting = new Promise((resolve, reject) => {
      vue.$http({
        method: 'post',
        url: window.config.server + '/api/v1/rooms/' + vue.propertyId + '/bedSetting',
        data: {
  ...
  const EditRoomFacilitiesAndOtherServices = vue => {
    const roomFacilitiesAndOtherServices = new Promise((resolve, reject) => {
      vue.$http({
        method: 'post',
        url: window.config.server + '/api/v1/rooms/' + vue.propertyId + '/roomFacilitiesAndOtherServices',
        data: {
