  ...
  export default {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            if (this.ruleForm.image !== '') {
              EditBanner(this).then((resolve) => {
                if (resolve.data.code === '200') {
                  this.$confirm('编辑banner成功', '提示', {
    ...
    created: function () {
      ...
      GetBanner(this).then((resolve) => {
        this.ruleForm.name = resolve.data.data.name
        this.ruleForm.image = resolve.data.data.image._id
        this.ruleForm.imageBtn = resolve.data.data.image._id
        this.loading = false
      }).catch((reject) => {
        window.publicFunction.error(reject, this)
      })
